...
function ambWindChart(ambData) {
    ...
    drawWindChart('Alta Mt Baldy', ambTime, ambData.wind_speed_set_1, ambData.wind_gust_set_1, ambDir);
...
function reyWindChart(reyData) {
    ...
    drawWindChart('Reynolds Peak', reyTime, reyWind, reyGust, reyDir);
...
function kslcWindChart(kslcData) {
    ...
    drawWindChart('KSLC', kslcTime, kslcWind, kslcGust, kslcDir);
...
function openWeatherForecast() { // https://openweathermap.org/api/one-call-api
    ...
    $.get(url, function(data) {
        console.log('KSLC Forecast:', data);
    });
}
