...
// --- Pinch zoom + pan for Keen Slider images (no double tap) ---
(function enablePinchZoom() {
  const sliderEl = document.getElementById("slider")
  const images = sliderEl.querySelectorAll(".keen-slider__slide img")
  ...
  images.forEach(img => {
    img.addEventListener("touchstart", e => {
      if (e.touches.length === 2) {
        ...
        currentScale = scale
        sliderEl.dataset.keenSliderDisabled = "true" // disable slide swipe
    ...
    img.addEventListener("touchmove", e => {
      if (e.touches.length === 2) {
        ...
        scale = Math.max(1, Math.min(scale, 3)) // clamp between 1x–3x
    ...
    img.addEventListener("touchend", e => {
      if (e.touches.length === 0) {
        if (scale <= 1.05) {
          ...
          sliderEl.removeAttribute("data-keen-slider-disabled")
        } else {
          zoomed = true
