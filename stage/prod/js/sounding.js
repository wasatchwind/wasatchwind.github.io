...
//   const today = new Date().toLocaleDateString("fr-CA", { year: "numeric", month: "2-digit", day: "2-digit" }); // fr-CA for needed format yyyy-mm-dd
//   // if (data[0] !== today || new Date().getHours() < 12) return;
...
//   const topOfLift = global.liftParams.topOfLift > surfaceAltMeters ? Math.round(global.liftParams.topOfLift).toLocaleString() : "Ø";
...
//     const { Temp_c, Altitude_ft } = data[index];
//     const thermalIndex = Temp_c - ((Altitude_ft / ftPerMeter - dalrYIntercept) / dalrSlope);
...
//             const { slope, yIntercept } = interpolate(t1, a1 / ftPerMeter, t2, a2 / ftPerMeter);
//             const targetX = (yIntercept - dalrYIntercept - (3 * dalrSlope)) / (dalrSlope - slope);
//             params.negative3 = a1 / ftPerMeter + (targetX - t1) * (a2 / ftPerMeter - a1 / ftPerMeter) / (t2 - t1);
...
//           params.topOfLift = a1 / ftPerMeter + (targetX - t1);
...
//       const currentAlt = (current.Altitude_ft * ftPerMeter) / 1000;
//       const bestAlt = (best.Altitude_ft * ftPerMeter) / 1000;
...
function processSounding(data, hiTemp, raobmsg) {
  console.log("Sounding data date:", raobmsg)
  const today = new Date().toLocaleDateString("fr-CA", { year: "numeric", month: "2-digit", day: "2-digit" }); // fr-CA for needed format yyyy-mm-dd
  if (raobmsg !== today || new Date().getHours() < 12) return;
